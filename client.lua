...
RegisterCommand('noclip', function(source, args)
    if aduty then
        noclip = not noclip
        if noclip then
		    TriggerEvent('notifications', "#1cf6f9", "Evolution Administration", "Du hast NoClip angeschaltet.")
        end
        if not noclip then
		    TriggerEvent('notifications', "#1cf6f9", "Evolution Administration", "Du hast NoClip ausgeschaltet.")
        end
    else
        TriggerEvent('notifications', "#1cf6f9", "Evolution Administration", "Du bist nicht im Admindienst.")
    end
end)
...
RegisterCommand('support', function(source, args)
    ...
    TriggerEvent('notifications', "#1df5fc", "TICKETSYSTEM", "Du hast ein Ticket abgesendet.")
end)
...
AddEventHandler('adminnotify', function(msg, title)
    ESX.TriggerServerCallback('rw:getGroup', function(group)
        if group == "superadmin" or group == "admin" or group == "mod" or group == "support" then
            TriggerEvent('notifications3', "#1df5fc", "REPORT - " .. title, "Grund: " .. msg)
        end
    end)
end)
...
AddEventHandler('adminnotify2', function(msg, title)
    ESX.TriggerServerCallback('rw:getGroup', function(group)
        if group == "superadmin" or group == "admin" or group == "mod" or group == "support" then
            TriggerEvent('notifications2', "#1df5fc", "TEAM-Chat - " .. title, msg)
        end
    end)
end)
    ...
    ESX.TriggerServerCallback('rw:getGroup', function(group)
        if group == "superadmin" or group == "admin" or group == "mod" or group == "support" or group == "_dev"then
            ...
            if aduty then
                ...
                headDots = false
                health = false
                TriggerEvent('notifications', "#fe1502", "ADMINISTRATION", "Du bist nun nicht mehr im Admindienst.")
                ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin)
                    TriggerEvent('skinchanger:loadSkin', skin)
                end)
            else
                aduty = true
                TriggerEvent('notifications', "#3f7d33", "Admindienst", "Du hast nun den Admindienst angetreten.")
                setUniform(group)
                godmode = true
                showNames = true
                headDots = true
